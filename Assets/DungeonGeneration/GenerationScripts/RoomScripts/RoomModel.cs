...
public class RoomModel : MonoBehaviour
{
    ...
    [HideInInspector] 
    public bool currentPlayerInRoom, otherPlayerEnteredRoom, roomCleared, roomIDAssigned, disableEnvironmentSpawn;
    
    [HideInInspector] 
    public int roomID, environmentSpawned;
    
    [HideInInspector] 
    public Vector3 networkRoomInfo; //roomID, roomType, roomSelection
    
    [HideInInspector] 
    public Vector2 environmentOrigin = new Vector2(0, 0); //spawnType, objectSpawned
    
    [HideInInspector] 
    public EnvironmentObjects environmentObjects;
    ...
    {
        if (!disableEnvironmentSpawn && environmentSpawnGroups.Length > 0 && (!hasConnectedRooms || hasConnectedRooms && isEnvironmentController))
            Spawner();
    }
    ...
    {
        if (DungeonGenerationV3.Instance && DungeonGenerationV3.Instance.spawnedRooms.roomModels.Contains(this)) 
            DungeonGenerationV3.Instance.spawnedRooms.roomModels.Remove(this);
    }
}
