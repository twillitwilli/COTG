...
public class EnemyAnimationController : MonoBehaviour
{
    ...
    public enum AnimationState { spawn, idle, walking, running, meleeAttack, rangedAttack, death, turnLeft, turnRight, takingDamage, frozen }
    ...
    public void ChangeAnimation(AnimationState animationState)
    {
        switch (animationState)
        {
            case AnimationState.spawn:
                if (spawnClip != null) { AnimationClip(spawnClip); }
                break;
    ...
    {
        if (melee) { currentAttack = Random.Range(0, meleeAttackClip.Length); }
        else { currentAttack = Random.Range(0, rangedAttackClip.Length); }
        return currentAttack;
    }

    public void AnimationClip(string animationClipName)
    {
        if (CoopManager.instance != null) { CoopManager.instance.coopEnemyController.ChangeAnimation(enemyController.spawnID, animationClipName); }
